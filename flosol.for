...
!##########################################################################
	...
	real (kind =8) :: wtime_cd,wtime_lpt,wtime_imbub
...
!	if(LSCALAR)   call sediment_init
	...
	if (myrank.eq.1) then
	 ...
	 !write(203,*)'Variables=it,C-D,PSolver,IBM,LPT,IMBub,Total'
	endif
	if (myrank.eq.0) then
	 open(unit=44, file='track_bubble.dat')
	 write(44,*)'Variables=time,z,up,vp,wp,ui,Fx,Fy,Fz,uslip'
	endif
...
!-----------------------------------------------------------------------
	...
	!no IMB nor LPT!!!!!!!!!!!!!!!!!!
	   ...
	   if (LIMBUB) then						!Brunho2018 Birmingham-CTR Stanford
		!write(6,*)wop_pt(1)
	   if (myrank.eq.0) then		
		if (tsnr.gt.0) 	then					!release particles and remove particles		
			if (mod(itime,tsnr).eq.0) call release_pt	
			call alloc_pt	
		elseif ((tsnr.eq.0).and.(PERIODIC)) then			!periodic cube
			call periodic_pt
		endif
		call update_IMBub
	   endif	
	   call MPI_bub							!MPI Lagrangian transfer
		!call Bub_deltah					!not necessary with the second algorithm
	   endif
	   ...
	   if (myrank.eq.1) wtime_imbub = MPI_WTIME ( )				!Brunho2013
	   if (LIMBUB) then							!Brunho2018	
        		call exchange(11)
        		call exchange(22)
        		call exchange(33)
			call bub_algorithm	
        		!if (LENERGY) call exchange(10)
			if (myrank.eq.0) call imb_bubbles			!master proc calculates bubs velocities and updates positions
			!call lag_pdf			!master proc claculates pdf at locations relative to the bubble
	     		CALL MPI_BARRIER(MPI_COMM_WORLD,ierr)
	   endif
	   if (myrank.eq.1) wtime_imbub = MPI_WTIME ( ) - wtime_imbub
