...
!##########################################################################
	...
	do ib=1,nbp
		...
		!if (dom_id(ib).eq.0) then
		Vcell = dom(ib)%dx*dom(ib)%dy*dom(ib)%dz	
	if (dom(ib)%xc(i).ge.0.d0.and.dom(ib)%xc(i).le.0.002) then
	if (dom(ib)%yc(j).ge.0.019.and.dom(ib)%yc(j).le.0.019) then
	if (dom(ib)%zc(k).ge.0.019.and.dom(ib)%zc(k).le.0.019) then
		dom(ib)%S(i,j,k)=
     & 		dom(ib)%S(i,j,k) + (1d-6/Vcell)*dt
		goto 25
		endif ; endif ; endif
		!endif
		enddo ; enddo ; enddo
